...
该命令固定输出 17 张图后自动退出：8 张全景（正背面、左右、俯仰、两个三分之四视角）、
4 张不显示尾焰的 RCS 安装座近景、4 张每次只显示当前 quad 的喷流/导流板斜视近景，
以及 1 张 DPS 喷管近景。

推进器改动还必须运行 viewer 单元测试。解析测试负责核查 16 个喷管/底座法向、8 个
水平喷流严格沿本体 `±X/±Z` 而非 quad 局部切向（与局部径向、切向各 45°）、12 个
自由喷流完整锥体避开共享简化外形件，以及 4 个下向喷流按实物构型有意撞击导流板并
向外排走。截图负责发现网格穿插、遮挡和观感问题，两者不能互相替代。
...
推进器回归还应明确覆盖 GDA 的 6° 圆锥目标限幅、`0.2°/s` 逐物理子步速率限制、
`Off` 保持、`reset` 回中，以及 2 ms 与 1 ms 物理步长的一秒摆角收敛对比。GDA 动态
不改变默认 2 ms × 10 的仿真/控制周期。
