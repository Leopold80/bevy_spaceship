...
当前代码同时实现两种运动学控制律。可视化演示默认使用旧控制律：
...
在 MuJoCo Apollo 动力学 demo 中，运动学姿态控制律作为外层环继续输出角速度指令：
...
随后内层角速度 PID 在 MuJoCo 刚体动力学上输出 body-frame 力矩：

$$
\boxed{
\tau=K_p(\omega_c-\omega)+K_i\int(\omega_c-\omega)\,dt+K_d\frac{d}{dt}(\omega_c-\omega).
}
$$

这个双层控制实验已经不再假设角速度可以完美跟踪，而是把 \(\tau\) 施加到 MuJoCo freejoint 刚体，由 MuJoCo 积分近似的刚体姿态动力学。它仍然是工程验证：当前实现包含内层 PID、积分限幅和力矩限幅，但还没有给出完整级联系统稳定性证明。
