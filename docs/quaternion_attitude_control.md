...
## 控制律一：\(q_{e0}q_{ev}\) 缩放反馈（legacy 参考）
...
保留在 `attitude_control::legacy` 的 Rust 参考实现只覆盖固定期望姿态 \(\omega_d=0\) 和标量增益 \(K_p=k_pI\)，没有实现时变目标的角速度前馈。此时控制律为：
...
这与 `legacy::scaled_quaternion_command` 中的历史实现一致，即：
...
当前 Rust 实现只覆盖固定目标和标量增益。运动学可视化演示和 MuJoCo 级联姿态演示都使用本控制律。
...
| 工程特性 | 大角度更柔和 | 大角度更积极 |
| 当前代码状态 | 仅在 `attitude_control::legacy` 保留参考 | 运动学与 MuJoCo 演示的唯一运行时控制律 |
...
运动学可视化演示使用 \(k_p=2.4\)，并且只使用常增益控制律：
...
随后内层角速度控制器向 MuJoCo 刚体输出 body-frame 力矩。它不是对跟踪误差求导的标准 PID，而是带测量微分、显式角速度阻尼和回算抗饱和的 PI-D。先按控制器的意图写为：
...
3. \(\theta_e\) 逐渐减小并趋近于零；
4. \(\Vert\omega_c\Vert\) 随 \(\theta_e\) 减小而减小，并最终趋近零。
