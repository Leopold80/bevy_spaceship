...
当前 Rust 实现只覆盖固定期望姿态 \(\omega_d=0\) 和标量增益 \(K_p=k_pI\)，没有实现时变目标的角速度前馈。此时控制律为：
...
这与 `attitude_command` 中的缩放反馈分支一致，即：
...
当前 Rust 实现同样只覆盖固定目标和标量增益。运动学可视化演示可通过 `C` 键在两种控制律之间切换；MuJoCo 级联姿态演示默认使用本控制律。
...
相对于缩放反馈，它的优势是接近 \(180^\circ\) 误差时仍然保留非零角速度指令；代价是角速度指令在大角度时更激进，并且在 \(180^\circ\) 的四元数表示切换边界不光滑。工程实现中通常需要角速度限幅或内层执行器约束；当前 MuJoCo 演示已对外环角速度指令限幅。
...
| 工程特性 | 大角度更柔和 | 大角度更积极 |
| 当前代码状态 | 已实现，运动学可视化默认 | 已实现，MuJoCo 级联演示默认；运动学演示可按 `C` 切换 |

---

## 工程验证设置

独立的 Bevy `attitude_demo` 只做运动学外环验证：忽略刚体动力学、惯量矩阵、力矩执行器、饱和约束和内层角速度环，并假设实际角速度能够完美跟踪角速度指令 \(\omega_c\)。

当前代码同时实现两种运动学控制律。运动学可视化演示使用 \(k_p=2.4\)，默认使用缩放反馈：
...
`C` 键切换控制律时会同时重置当前场景、仿真时间和日志；日志文件会被重新创建。

在 MuJoCo Apollo 外形单刚体 demo 中，外环默认使用常增益反馈 \(k_p=5.0\)，并对角速度指令做幅值限制：
...
随后内层角速度控制器向 MuJoCo 刚体输出 body-frame 力矩。它不是对跟踪误差求导的标准 PID，而是带测量微分、显式角速度阻尼和条件积分的 PI-D。先按控制器的意图写为：
...
其中 \(\dot\omega_b\) 由相邻控制周期的测量角速度差分获得，微分对测量而非误差作用，以避免外环指令变化带来的 derivative kick；二阶 biquad 低通则用于滤除差分测量中的高频成分。积分向量的幅值上限为 \(6.4\)；当候选力矩超限时冻结积分，最终力矩向量限幅为 \(52\,000\)。

上式中的 \(\omega_b\) 是机体系角速度。MuJoCo freejoint 的旋转 `qvel[3..6]` 原生就在刚体局部坐标系中表达，因此当前实现直接使用 \(\omega_b=\omega_{qvel}\)，不再乘以 \(R(q)^\top\)。MuJoCo demo 从非恒等姿态和与其旋转轴不对齐的非零机体系角速度出发；回归测试既检查该场景的最终姿态/角速度收敛，也检查 2 s 时的短时收敛上限，因此恢复旧的重复坐标变换会直接使测试失败。另一个测试直接检查非恒等姿态下的阻尼力矩轴向。详见 [README 的坐标系约定与挑战收敛场景](../README.md#坐标系约定与挑战收敛场景)。

这个双层控制实验已经不再假设角速度可以完美跟踪，而是把 \(\tau\) 施加到 MuJoCo freejoint 刚体，由 MuJoCo 积分刚体姿态动力学。它仍然是工程验证：当前还没有给出完整级联系统稳定性证明。

当前 CSV 日志只属于运动学演示，MuJoCo 演示尚未记录这些量。运动学演示写入 `logs/attitude_kinematics.csv`，记录：
...
CSV 中的 `omega_norm`、`omega_x`、`omega_y`、`omega_z` 都是角速度指令 \(\omega_c\)，不是动力学模型的测量角速度。
...
3. \(\theta_e\) 逐渐减小并趋近于零；
4. \(\Vert\omega_c\Vert\) 最终趋近零；常增益反馈下它会随 \(\theta_e\) 减小而单调减小，缩放反馈下 \(\Vert\omega_c\Vert=\frac{k_p}{2}\sin\theta_e\)，因此当初始误差大于 \(90^\circ\) 时可能先增大、再减小；
5. 对接近但不等于 \(180^\circ\) 的初始误差，缩放反馈仍会收敛，但大角度附近会更慢。

---

## 展望

上面的 Lyapunov 推导基于一个较强假设：角速度环可以完美跟踪角速度指令。这在运动学外环验证中是合理的简化。当前 MuJoCo 路径已经引入刚体姿态动力学：

$$
J\dot\omega+\omega\times J\omega=\tau,
$$

并且已实现转动惯量、连续 body-frame 力矩输入、角速度内环、角速度指令限幅、积分限幅和力矩限幅。下一步应当考虑：

1. 将连续合力矩约束细化为离散 RCS 喷口、最小脉宽和控制分配约束；
2. 扰动力矩、测量噪声和模型不确定性；
